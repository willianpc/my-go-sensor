...
func WrapFunctionHandler(sensor *instana.Sensor, handler http.HandlerFunc) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, req *http.Request) {
		...
		if trigger == unknownType || method == "" {
			span.SetTag("azf.error", "unknown type")
			span.LogFields(otlog.Object("error", err))
		}
		...
		traceCtx, cancelTraceCtx := context.WithCancel(req.Context())
		...
		originalDeadline, deadlineDefined := req.Context().Deadline()
		if deadlineDefined {
			traceCtx, cancelTraceCtx = context.WithDeadline(req.Context(), originalDeadline.Add(-azfTimeoutThreshold))
		}
